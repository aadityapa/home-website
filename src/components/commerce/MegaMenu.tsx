...
export function MegaMenu({ open, onClose }: MegaMenuProps) {
  ...
  return (
    <div
      className="absolute left-0 right-0 top-full z-50 hidden border-b border-white/10 bg-noir-950/98 shadow-2xl backdrop-blur-2xl lg:block"
      onMouseLeave={onClose}
    >
      <div className="mx-auto grid max-w-6xl gap-8 px-8 py-8 md:grid-cols-4">
        {categories.map((cat) => {
        ...
        <div className="flex flex-col justify-between rounded-xl border border-amber-500/20 bg-amber-500/5 p-6">
          <div>
            ...
            <p className="mt-2 font-sans text-sm text-noir-300">Browse the full catalog with filters and quick add.</p>
          </div>
          <Link
            href="/shop"
            onClick={onClose}
